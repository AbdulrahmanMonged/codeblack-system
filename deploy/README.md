...
- `scripts/publish_frontend.sh` - build frontend and publish to `/var/www/codeblack`
...
```bash
WEB_ROOT=/var/www/codeblack deploy/scripts/publish_frontend.sh
```

3. Install Caddy config
```bash
DOMAIN=codeblack.example.com WEB_ROOT=/var/www/codeblack deploy/scripts/install_caddy_site.sh
```

4. Install and enable systemd services
```bash
APP_DIR=/path/to/codeblack-bot-recovered-20260220 APP_USER=root APP_GROUP=root START_SERVICES=1 deploy/scripts/install_systemd.sh
```
