...
- Frontend static files: **`/var/www/codeblack`** (`index.html` + assets from `frontend/dist`)
...
## Services
- `codeblack-backend.service` -> FastAPI API
- `codeblack-celery.service` -> Celery worker
- `codeblack-celery-beat.service` -> Celery beat scheduler
- `codeblack-bot.service` -> Discord bot

All unit templates are in `deploy/systemd/` and are rendered by `deploy/scripts/install_systemd.sh`.

## Caddy Routing
- Static SPA files served from `/var/www/codeblack`
...
- Output synced to `/var/www/codeblack`

5. Install Caddy site config
- Render and install from template:
  - `DOMAIN=your-domain.com WEB_ROOT=/var/www/codeblack deploy/scripts/install_caddy_site.sh`
...
- Validate:
  - `systemctl status codeblack-backend codeblack-celery codeblack-celery-beat codeblack-bot`
  - `curl http://127.0.0.1:8000/api/v1/system/health`
  - `curl -I https://your-domain.com`

## Operational Notes
- Keep backend and bot in separate venvs to avoid dependency conflicts.
- Caddy serves only built frontend files from `/var/www/codeblack`.
...
- Frontend: restore previous `/var/www/codeblack` backup.
