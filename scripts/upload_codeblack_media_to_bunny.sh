...
DRY_RUN=0
SOURCE_DIR="REDACTED-media"
ENV_FILE=".env"
KEY_PREFIX=""
MANIFEST_FILE="REDACTED-media/cdn-manifest.tsv"

usage() {
  cat <<'USAGE'
Usage: scripts/upload_REDACTED_media_to_bunny.sh [options]

Options:
  --dry-run                 Print what would be uploaded without sending requests.
  --source <dir>            Source media directory (default: REDACTED-media).
  --env-file <file>         Env file to read Bunny config from (default: .env).
  --prefix <path>           Optional key prefix inside Bunny zone.
  --manifest <file>         Output manifest path (default: REDACTED-media/cdn-manifest.tsv).
